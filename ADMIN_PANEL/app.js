...
async function loadInterns() {
    ...
    document.getElementById('interns-table-body').innerHTML = '<tr><td colspan="5">TESTING...</td></tr>';
    try {
        const json = await fetchApi('/interns');
        if (!json.success) {
            document.getElementById('interns-table-body').innerHTML = '<tr><td colspan="5" style="color:red;">Error: ' + json.message + '</td></tr>';
            return;
        } if (!json.success) alert("Load Interns Failed: " + JSON.stringify(json));
        
        if(json.success) {
            const tbody = document.getElementById('interns-table-body');
            tbody.innerHTML = '';
            if(json.data.length === 0) {
                tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:30px; color:var(--text-light);">No listing interns registered yet. Click \'Add Listing Intern\' to generate an onboarding upload link.</td></tr>';
                return;
            }
            json.data.forEach(i => {
                const tr = document.createElement('tr');
                tr.innerHTML = `
                    ...
                    <td style="text-align:right;">
                        <div style="display:flex; justify-content:flex-end; gap:6px;">
                            <button class="icon-btn" onclick="toggleInternStatus('${i.id}', '${i.status === 'ACTIVE' ? 'SUSPENDED' : 'ACTIVE'}')" title="${i.status === 'ACTIVE' ? 'Suspend Access' : 'Reactivate'}">
                                <i data-feather="${i.status === 'ACTIVE' ? 'pause-circle' : 'play-circle'}"></i>
                            </button>
                            <button class="icon-btn" onclick="openChangePasswordModal('${i.id}')" title="Reset Password">
                                <i data-feather="key"></i>
                            </button>
                            <button class="icon-btn" onclick="openDeleteInternModal('${i.id}', '${i.name}')" title="Delete Intern" style="color:#ef4444;">
                                <i data-feather="trash-2"></i>
                            </button>
                        </div>
                    </td>
...
async function loadInternListings() {
    try {
        ...
        if(json.success) {
            window.currentInternListings = json.data; // Store ALL listings globally for the modal
