        ...
        if (data.success) {
            ...
            data.messages.forEach(msg => {
                ...
                div.innerHTML = `
                    ...
                    <div style="font-size: 10px; text-align: right; margin-top: 4px; color: ${timeColor}; opacity: 0.9;">${new Date(msg.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div>
                `;
...
window.sendAdminChatMessage = async function() {
    ...
    const div = document.createElement('div');
    div.style.cssText = `align-self: flex-end; background-color: #EAE6DF; color: #3B2115; padding: 10px 14px; border-radius: 8px; max-width: 70%; margin-bottom: 8px; border: 1px solid var(--border-color);`;
    div.innerHTML = `
        <div style="font-size: 14px; font-weight: 500;">${msg}</div>
        <div style="font-size: 10px; text-align: right; margin-top: 4px; color: #6B5B45; opacity: 0.9;">Just now</div>
    `;
    container.appendChild(div);
    container.scrollTop = container.scrollHeight;
...
async function editProperty(id) {
    try {
        ...
        if (res.success) {
            ...
            if (subSelect) subSelect.value = subValue;
...
if(document.getElementById('property-form')) document.getElementById('property-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const sub = subElem ? subElem.value : '';
...
document.querySelectorAll('.nav-item').forEach(item => {
    item.addEventListener('click', () => {
        ...
        if (targetId === 'interns-content') {
            if (typeof window.loadInterns === 'function') {
                window.loadInterns();
            } else {
                loadInterns();
            }
            if (typeof window.loadInternListings === 'function') {
                window.loadInternListings();
            } else {
                loadInternListings();
            }
        }
