...
async function fetchApi(endpoint, options = {}) {
    ...
    // Redirect intern endpoints to the new backend paths
    if (endpoint.startsWith('/interns')) endpoint = '/admin' + endpoint;
    else if (endpoint.startsWith('/intern-listings')) endpoint = endpoint.replace('/intern-listings', '/admin/intern-properties');
    else if (endpoint.startsWith('/intern-properties')) endpoint = '/admin' + endpoint;
...
async function loadInternsChat() {
    try {
        const data = await fetchApi('/admin/interns');
        if (data.success) {
            const listContainer = document.getElementById('chat-intern-list');
            listContainer.innerHTML = '';
            
            if (data.data.length === 0) {
                listContainer.innerHTML = '<div class="empty-state">No interns available.</div>';
                return;
            }
            
            data.data.forEach(intern => {
                const div = document.createElement('div');
                div.style.cssText = `padding: 10px; border-radius: 8px; cursor: pointer; margin-bottom: 5px; transition: background 0.2s;`;
                div.className = 'intern-chat-item';
                div.onclick = () => selectInternChat(intern.id, div, intern.name);
                div.innerHTML = `
                    <div style="font-weight: 600;">${intern.name}</div>
                    <div style="font-size: 12px; color: var(--text-secondary);">${intern.intern_id}</div>
                `;
                listContainer.appendChild(div);
            });
        }
    } catch (err) {
        console.error('Failed to load interns for chat', err);
    }
}

async function selectInternChat(internId, element, internName) {
    ...
    document.getElementById('chat-header').innerHTML = `Chat with <strong>${internName}</strong>`;
    
    await loadAdminChat();
}

async function loadAdminChat() {
    if (!currentChatInternId) return;
    try {
        const data = await fetchApi(`/admin/interns/${currentChatInternId}/chat`);
        if (data.success) {
            const container = document.getElementById('admin-chat-messages');
            container.innerHTML = '';
            
            if (!data.messages || data.messages.length === 0) {
                container.innerHTML = '<div style="text-align:center; color:var(--text-secondary); margin-top:2rem;">Start a conversation with this intern</div>';
                return;
            }
            
            data.messages.forEach(msg => {
                const align = msg.sender_type === 'admin' ? 'flex-end' : 'flex-start';
                const bg = msg.sender_type === 'admin' ? 'var(--primary-color)' : 'var(--bg-main)';
                const color = msg.sender_type === 'admin' ? '#fff' : 'var(--text-primary)';
...
window.sendAdminChatMessage = async function() {
    if (!currentChatInternId) {
        alert('Please select an intern to chat with first.');
        return;
    }
    ...
    const div = document.createElement('div');
    div.style.cssText = `align-self: flex-end; background: var(--primary-color); color: #fff; padding: 10px 14px; border-radius: 8px; max-width: 70%; margin-bottom: 8px;`;
    div.innerHTML = `
    ...
    try {
        await fetchApi(`/admin/interns/${currentChatInternId}/chat`, {
            method: 'POST',
            body: JSON.stringify({ message: msg })
        });
        // Optional: reload chat silently
    } catch (err) {
        alert('Failed to send message');
    }
};

// Hook into nav clicks for chat
document.querySelectorAll('.nav-item').forEach(item => {
    item.addEventListener('click', () => {
        const targetId = item.getAttribute('data-target');
        if (targetId === 'interns-chat-content') {
            loadInternsChat();
        }
    });
});
...
async function editProperty(id) {
    try {
        ...
        if (res.success) {
            ...
            if (catSelect) {
                if (['FLAT', 'PG', ''].includes(catValue)) {
                    catSelect.value = catValue;
                } else {
                    catSelect.value = 'OTHER';
                    document.getElementById('prop-category-custom').value = catValue;
                }
            }
            ...
            if (subSelect) {
                if (['1 BHK', '2 BHK', '3 BHK', '3BHK+', ''].includes(subValue)) {
                    subSelect.value = subValue;
                } else {
                    subSelect.value = 'OTHER';
                    document.getElementById('prop-subcategory-custom').value = subValue;
                }
            }
...
if(document.getElementById('property-form')) document.getElementById('property-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const media = { photos: photosUrls, video: videoUrl };
        const rawSpecs = document.getElementById('prop-specs').value.split(',').map(s => s.trim()).filter(s => s);
        
        const catElem = document.getElementById('prop-category');
        const subElem = document.getElementById('prop-subcategory');
        let cat = catElem ? catElem.value : '';
        if (cat === 'OTHER') cat = document.getElementById('prop-category-custom').value.trim();
        let sub = subElem ? subElem.value : '';
        if (sub === 'OTHER') sub = document.getElementById('prop-subcategory-custom').value.trim();

        if (!cat) { alert('Please enter a category.'); btn.textContent = originalBtnText; btn.disabled = false; return; }
        if (!sub) { alert('Please enter a subcategory.'); btn.textContent = originalBtnText; btn.disabled = false; return; }
...
window.handleCreateIntern = async function(e) {
    ...
    try {
        const res = await fetchApi('/admin/interns', {
            method: 'POST',
...
async function handleCreateIntern(event) {
    ...
    btn.innerHTML = '<div class="spinner" style="width:14px;height:14px;border:2px solid #fff;border-top-color:transparent;border-radius:50%;animation:spin 1s linear infinite;display:inline-block;vertical-align:middle;margin-right:6px;"></div> Creating...';
    btn.disabled = true;

    const internId = 'INT' + Math.floor(10000 + Math.random() * 90000);
    const data = {
        ...
        territory: document.getElementById('int-territory').value,
        monthly_target: document.getElementById('int-target').value
    };

    try {
        const res = await fetchApi('/admin/interns', { method: 'POST', body: JSON.stringify(data) });
        if (!res.success) throw new Error(res.message);

        // Show success
        document.getElementById('create-intern-form').style.display = 'none';
        document.getElementById('intern-success-card').style.display = 'block';
        document.getElementById('success-intern-id').textContent = data.intern_id; // Using the generated ID
        document.getElementById('success-intern-password').textContent = document.getElementById('int-password').value;
        loadInterns();
    } catch(err) {
...
if(document.getElementById('change-password-form')) document.getElementById('change-password-form').addEventListener('submit', async (e) => {
    ...
    try {
        const json = await fetchApi('/interns/' + internToReset + '/reset-password', {
            method: 'POST',
            body: JSON.stringify({ new_password: newPass })
        });
        if(json.success) {
            closeChangePasswordModal();
            alert('Password reset successfully. The intern can now log in with the new password.');
        } else {
...
async function executeDeleteListing() {
    try {
        const res = await fetchApi('/admin/intern-properties/' + listingToDelete, { method: 'DELETE' });
        const data = await res.json();
        if(data.success) {
...
function openReviewModal(id) {
    ...
    if (listing.is_published) {
        ...
        // we can construct the category URL or the exact property URL
        // In the main site, exact property is allproperties.html?id=XXX
        // However wait, the id might be the new properties.json ID which we don't have easily, but category we do!
        // The instructions say "opens the correct public property page if that functionality already exists."
        // Actually, let's just link to the category page it was published to.
        const catUrl = listing.published_subcategory ? 
            'https://brick-stone-frontend.vercel.app/FRONTEND/allproperties.html?category=' + encodeURIComponent(listing.published_subcategory) :
            'https://brick-stone-frontend.vercel.app/FRONTEND/allproperties.html';
        if(viewMain) viewMain.href = catUrl;
    } else {
...
window.submitReview = async function(status) {
    ...
    try {
        const endpoint = '/intern-properties/' + window.currentReviewListingId + '/review';
        const body = { status: status, admin_feedback: feedback };
        ...
        if (status === 'APPROVED') {
            alert('Listing Approved and Published to the main website!');
        }
    } catch(err) {
        console.error(err);
...
window.executeApproveAndPublish = async function(e) {
    ...
    try {
        ...
        console.error(err);
        alert('Exception while publishing: ' + err.message);
    } finally {
...
function exportLeads(type) {
    ...
    let tableId = activeTab === 'enquiries-tab' ? 'enquiries-tbody' : 'contacts-tbody';
    let title = activeTab === 'enquiries-tab' ? 'Property_Enquiries' : 'General_Contacts';
    ...
    let rows = [];
    let headers = activeTab === 'enquiries-tab' 
        ? ['Date', 'Property Interest', 'Phone', 'Email', 'Status'] 
    ...
    Array.from(tbody.children).forEach(tr => {
        ...
        let row = [];
        if (activeTab === 'enquiries-tab') {
            row.push(cells[0].innerText.trim()); // Date
            row.push(cells[1].innerText.trim()); // Property
            // Split Phone and Email from the 3rd column
            let contactDivs = cells[2].querySelectorAll('div');
            if (contactDivs.length >= 2) {
                row.push(contactDivs[0].innerText.trim());
                row.push(contactDivs[1].innerText.trim());
            } else {
                row.push(cells[2].innerText.trim());
                row.push("");
            }
            row.push(cells[3].innerText.trim()); // Status
        } else {
            row.push(cells[0].innerText.trim()); // Date
            row.push(cells[1].innerText.trim()); // Name
            // Contact
            let contactDivs = cells[2].querySelectorAll('div');
            if (contactDivs.length >= 2) {
                row.push(contactDivs[0].innerText.trim());
            ...
            row.push(cells[3].innerText.trim()); // Message
        }
        rows.push(row);
    });

    const dateStr = new Date().toISOString().split('T')[0];
    const filename = `Brickstone_${title}_${dateStr}`;
    ...
    if (type === 'excel') {
        ...
        <h2>${title.replace('_', ' ')} - Brickstone Real Estate</h2>
        ...
        rows.forEach((r, i) => {
            ...
            r.forEach(c => {
                htmlContent += i === 0 
                    ? `<th style='background-color: #f3f4f6; padding: 8px; text-align: left;'>${c}</th>`
        ...
        // Build a clean invisible table for PDF generation
        let div = document.createElement('div');
        div.style.padding = '20px';
        div.style.fontFamily = 'Helvetica, sans-serif';
        div.innerHTML = `
            <h2 style="color: #3b2f29; border-bottom: 2px solid #d4af37; padding-bottom: 10px; margin-bottom: 20px;">Brickstone Real Estate - ${title.replace('_', ' ')}</h2>
            <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
                <thead>
                    <tr style="background-color: #f8fafc; text-align: left;">
                        ${rows[0].map(h => `<th style="padding: 10px; border-bottom: 2px solid #e2e8f0; color: #475569;">${h}</th>`).join('')}
                    </tr>
                </thead>
                <tbody>
                    ${rows.slice(1).map(r => `
                        <tr>
                            ${r.map(c => `<td style="padding: 10px; border-bottom: 1px solid #e2e8f0; color: #1e293b;">${c}</td>`).join('')}
                        </tr>
                    `).join('')}
                </tbody>
            </table>
            <div style="margin-top: 20px; font-size: 10px; color: #94a3b8; text-align: right;">Generated on ${new Date().toLocaleString()}</div>
        `;
