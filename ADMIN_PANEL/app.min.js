...
window.submitReview = async function(status) {
    ...
    try {
        let endpoint = '';
        let body = {};
        if (status === 'APPROVED') endpoint = '/intern-properties/' + window.currentReviewListingId + '/publish';
        else if (status === 'CHANGES REQUESTED') {
            endpoint = '/intern-properties/' + window.currentReviewListingId + '/request-changes';
            body = { note: feedback };
        } else if (status === 'REJECTED') {
            endpoint = '/intern-properties/' + window.currentReviewListingId + '/reject';
            body = { reason: feedback || 'No reason provided' };
        }
...
window.executeApproveAndPublish = async function(e) {
    ...
    try {
        const res = await fetchApi('/admin/intern-properties/' + window.currentReviewListingId + '/review', {
            method: 'PATCH',
            body: JSON.stringify({ 
                status: 'APPROVED', 
